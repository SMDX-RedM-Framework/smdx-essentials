...
local function updatePermissionLevel()
    SMDXCore.Functions.TriggerCallback('smdx-afkkick:server:GetPermissions', function(userGroups)
...
RegisterNetEvent('SMDXCore:Client:OnPermissionUpdate', function()
    updatePermissionLevel()
end)
                        ...
                        if time then
                            if time > 0 then
                                ...
                                if _type == 'minutes' then
                                    SMDXCore.Functions.Notify(Lang:t('afk.will_kick') .. math.ceil(time / 60) .. Lang:t('afk.time_minutes'), 'error', 10000)
                                elseif _type == 'seconds' then
                                    SMDXCore.Functions.Notify(Lang:t('afk.will_kick') .. time .. Lang:t('afk.time_seconds'), 'error', 10000)
                                end
                                time -= 1
                            else
