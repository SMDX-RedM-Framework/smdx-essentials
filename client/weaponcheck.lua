...
CreateThread(function()	-- Check if player has weapon in inventory --
    while true do
        ...
        local WeaponData = SMDXCore.Shared.Weapons[weapon]
        if WeaponData ~= nil and WeaponData["name"] ~= "weapon_unarmed" then
            ...
            if weaponGroup ~= `group_thrown` then
                ...
                local hasItem = SMDXCore.Functions.HasItem(WeaponData["name"])
                --print(hasItem)
                if not hasItem then
                    SetCurrentPedWeapon(player, `WEAPON_UNARMED`, true)
                    RemoveAllPedWeapons(player, true, true)
                    TriggerServerEvent("smdx-log:server:CreateLog", "anticheat",  Lang:t('log.weapon_removed'), "orange", '** @staff ** '..fullname..' had a weapon on them that they did not have in his inventory : anticheat has removed the weapon')
                end
            end
        end
    end
