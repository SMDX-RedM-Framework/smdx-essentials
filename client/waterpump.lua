...
exports['smdx-target']:AddTargetModel(WaterOutlet, {
    options = {
        {
            type = "client",
            event = 'smdx-waterpump:client:drinking',
            icon = "far fa-eye",
...
RegisterNetEvent('smdx-waterpump:client:drinking', function()
    if isBusy then
        return
    else
        isBusy = not isBusy
        ...
        Wait(5000)
        TriggerServerEvent("SMDXCore:Server:SetMetaData", "thirst", SMDXCore.Functions.GetPlayerData().metadata["thirst"] + math.random(25, 50))
        ClearPedTasks(ped)
        isBusy = not isBusy
    end
end)
