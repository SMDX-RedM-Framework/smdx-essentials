...
AddEventHandler('smdx-essentials:client:DrinkingAnimation', function()
...
RegisterNetEvent("consumables:client:Drink", function(itemName)
    ...
    TriggerEvent('smdx-essentials:client:DrinkingAnimation')
    ...
    SMDXCore.Functions.Progressbar("drinking-water", "Drinking from the Flask", 4500, false, true,
    {
        ...
        TriggerServerEvent("SMDXCore:Server:SetMetaData", "thirst", SMDXCore.Functions.GetPlayerData().metadata["thirst"] + ConsumeablesDrink[itemName])

        SMDXCore.Functions.Notify("You drank from your Flask!", 'success', 3000)
    end)
end)
...
RegisterNetEvent("consumables:client:DrinkWhiskey", function(itemName)
    ...
    local coords = GetEntityCoords(ped)
    local boneIndex = GetEntityBoneIndexByName(ped, "SKEL_R_Finger00")
    local prop = CreateObject(GetHashKey("p_whiskeybottle01x"), coords, true, true, true)
    isBusy = not isBusy
    ...
    SMDXCore.Functions.Progressbar("drinking-whiskey", "Dricker ur flaskan...", 4500, false, true,
    {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        ...
        SMDXCore.Functions.Notify("You drank from the bottle!", 'success', 3000)
    end)
end)
...
RegisterNetEvent("consumables:client:DrinkBeer", function(itemName)
    ...
    local coords = GetEntityCoords(ped)
    local boneIndex = GetEntityBoneIndexByName(ped, "SKEL_R_Finger00")
    local prop = CreateObject(GetHashKey("p_beerbottle01x"), coords, true, true, true)
    isBusy = not isBusy
    ...
    SMDXCore.Functions.Progressbar("drinking-beer", "Dricker en öl...", 4500, false, true,
    {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        ...
        SMDXCore.Functions.Notify("Du drack en god öl!", 'success', 3000)
    end)
end)
...
RegisterNetEvent("consumables:client:Smoke", function(itemName)
    if isBusy then
        ...
        Wait(sleep)
        TriggerEvent("inventory:client:ItemBox", SMDXCore.Shared.Items[itemName], "remove")
...
RegisterNetEvent("consumables:client:Eat", function(itemName)
    if isBusy then
        ...
        Wait(sleep)
        TriggerEvent("inventory:client:ItemBox", SMDXCore.Shared.Items[itemName], "remove")
        TriggerServerEvent("SMDXCore:Server:SetMetaData", "hunger", SMDXCore.Functions.GetPlayerData().metadata["hunger"] + ConsumeablesEat[itemName])
...
RegisterNetEvent('consumables:client:UseFieldBandage', function()
    local ped = PlayerPedId()
    local hasItem = SMDXCore.Functions.HasItem('fieldbandage', 1)
    if hasItem then
        SMDXCore.Functions.Progressbar("use_fieldbandage", "Using Field Bandage..", 4000, false, true, {
            ...
            StopAnimTask(ped, "mini_games@story@mob4@heal_jules@bandage@arthur", "bandage_fast", 1.0)
            SMDXCore.Functions.Notify(Lang:t('error.canceled'), "error")
        end)
    else
        SMDXCore.Functions.Notify('you don\'t have any field bandages!', 'error')
    end
end)
